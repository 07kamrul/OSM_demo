...
class ApiService {
  ...
  Future<dynamic> getAllUserLocations() async {
    final response = await http.get(Uri.parse('$baseUrl/UserLocation/GetAllUserLocations'));
    return jsonDecode(response.body);
  }

  // Get User Location by ID
  Future<dynamic> getUserLocationById(int id) async {
    final response = await http.get(Uri.parse('$baseUrl/UserLocation/GetUserLocation?id=$id'));
    return jsonDecode(response.body);
  }
  ...
  Future<dynamic> addUserLocation(Map<String, dynamic> locationData) async {
    final response = await http.post(
      Uri.parse('$baseUrl/UserLocation/AddUserLocation'),
      headers: {'Content-Type': 'application/json'},
  ...
  Future<dynamic> updateUserLocation(int id, Map<String, dynamic> locationData) async {
    final response = await http.put(
      Uri.parse('$baseUrl/UserLocation/UpdateUserLocation?id=$id'),
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode(locationData),
    );
    return jsonDecode(response.body);
